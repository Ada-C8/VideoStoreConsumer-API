class MovieWrapper
  ...
  def self.construct_movie(api_result)
    Movie.new(
      ...
      image_url: (api_result["poster_path"] ? self.construct_image_url(api_result['poster_path']) : DEFAULT_IMG_URL), #(api_result["poster_path"] ? self.construct_image_url(api_result["poster_path"]) : nil),
